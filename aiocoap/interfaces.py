...
class EndpointAddress(metaclass=abc.ABCMeta):
    ...
    0x1234>, wherever that's connected to" (simple6 or TCP) or "with
...
class Request(metaclass=abc.ABCMeta):
    ...
    instantiated directly, but generated by a :meth:`RequestProvider.request`
    method."""

    response = """A future that is present from the creation of the object and \
        fulfilled with the response message.
...
class Resource(metaclass=abc.ABCMeta):
    ...
    async def _render_to_pipe(self, pipe: Pipe) -> None:
        ...
        if await self.needs_blockwise_assembly(req):
            ...
            # Note that unless the lambda gets called, we're not fully
