...
class BlockwiseResponder:
    """A BlockwiseResponder encapsulates all state held to process a sequence
    of blockwise requests that may be related, except its own registration in a
    context (or similar) which is managed from the outside.

    It dispatches all requests sent to it in render_to_plumbing_request by
    assembling full payloads, having the context answer them, and handing out
    immediate responses to the partial requests.

    The BlockwiseResponder can and will answer multiple independent blockwise
    operations in sequence. It will shut itself down when an operation is
    completed, tough, so multiple operations on the same responder only happen
    when the client requests a new operation before the last one is concluded.
    """

    # FIXME: time out yourself

    def __init__(self, context, removal_cb):
        self._context = context
        self._removal_cb = removal_cb
        self._jumborequest_task = None
        self._reset()

    def _reset(self):
        self._block1 = None
        if self._jumborequest_task is not None:
            self._jumborequest_task.cancel()
            self._jumborequest_task = None
        self._first_response_available = asyncio.Future()
        self._latest_event = None
        self._new_event = asyncio.Future()

    @staticmethod
    def get_key(request):
        """Produce a hashable value that is suitable for keeping a list of
        ongoing blockwise operations"""

        # FIXME: it would be more efficient to not send the URI through the URI
        # construction frobnicator, but rather identify all relevant opts and
        # keep their tuples.
        # FIXME: Request-Tag goes just here
        return (request.remote, request.get_request_uri())

    async def render_to_plumbing_request(self, plumbing_request, cancellation_future):
        request = plumbing_request.request

        if request.opt.block2 and request.opt.block2.block_number != 0:
            if request.opt.block1 is not None:
                # FIXME: raise BadOption for better clarity
                plumbing_request.add_response(
                    Message(
                        code=BAD_OPTION,
                        payload=b"block1 and block2!=0 present"),
                    is_last=True)
                return
        ...
        if request.opt.block1 is not None:
            if request.opt.block1.block_number == 0:
                self._reset()
                self._block1 = request
            else:
                self._block1._append_request_block(request)
            if self._block1.opt.block1.more:
                plumbing_request.add_response(
                        Message(
                            code=CONTINUE,
            ...
            request = self._block1

        self._block1 = None
        # by now, all block1 stuff has been dealt with

        if self._jumborequest_task is None:
            # this will typically be the case when the current request is the
            # last block1 part or there is no block1 involved, but i see no
            # reason right now to enforce sequential extraction of block2 parts
            # in a combined block1+block2 operation as long as the same code
            # path is used for all cases.
            self._jumborequest_task = asyncio.get_event_loop().create_task(self._run_outer(request))
            jumborequest_created_here = True

            # FIXME this seems not to be working, server observations on
            # blockwise resources don't stop yet
            cancellation_future.add_done_callback(lambda x: self._jumborequest_task.cancel())
        else:
            jumborequest_created_here = False

        await self._first_response_available

        # by now, we can always assume that a response is available, and serve
        # its chunks.

        # non-block2=0 messages will run through this only once, but the first
        # block2 message will have this running as part of its notification
        # chain
        while True:
            ev = self._latest_event

            if ev.exception is not None:
                raise ev.exception

            can_continue = (
                        request.opt.block2 is None \
                        or request.opt.block2.block_number == 0) \
                    and request.opt.observe == 0 \
                    and not ev.is_last

            message = ev.message

            # FIXME: the criterion should rather be "ask the remote for whether
            # it thinks this will be under its MTU"...
            if request.opt.block2 is None and len(message.payload) < 1100:
                plumbing_request.add_response(message, is_last=not can_continue)
            else:
                raise RuntimeError("Can't fragment properly yet")

            if not can_continue:
                break

            self._new_event = asyncio.Future()
            await self._new_event

        if jumborequest_created_here:
            # don't fully reset -- there might still be interest to pull out
            # other blocks.
            self._jumborequest_task.cancel()

        # FIXME: make sure this is set to concluded if this was a last blcok
        # and there is no running jumborequest task

    async def _run_outer(self, request):
        try:
            pr = PlumbingRequest(request)
            await self._run_inner(pr)
        except Exception as e:
            if not self._first_response_available:
                self._first_response_available.set_exception(e)
            else:
                self._context.log.exception("Error late inside a blockwise"
                                        " observation: %s", e, exc_info=e)
                pass
        finally:
            self._reset()
            pr.stop_interest()

    async def _run_inner(self, plumbing_request):
        self._context.render_to_plumbing_request(plumbing_request, _handle_blockwise=False)

        while True:
            ev = await plumbing_request._events.get()

            if not self._first_response_available.done():
                self._first_response_available.set_result(None)
            else:
                self._new_event.set_result(None)
